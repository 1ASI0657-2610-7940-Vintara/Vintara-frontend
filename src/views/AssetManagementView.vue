...
const fetchSupplies = async () => {
  ...
  try {
    const { data } = await apiClient.get(SUPPLIES_PATH)
    supplies.value = Array.isArray(data) ? data : (data.items ?? [])
  } catch (error) {
...
const saveSupply = async () => {
  ...
  try {
    let savedId
    if (isEditMode.value) {
      const putPayload = { id: editingId.value, ...basePayload }
      const { data } = await apiClient.put(`${SUPPLIES_PATH}/${editingId.value}`, putPayload)
      const index = supplies.value.findIndex((s) => s.id === editingId.value)
      if (index !== -1) {
        supplies.value[index] = data ?? { ...supplies.value[index], ...basePayload }
      }
      savedId = editingId.value
      toastStore.success(`Insumo "${basePayload.supplyName}" actualizado.`, 'Actualizado')
    } else {
      const { data } = await apiClient.post(SUPPLIES_PATH, basePayload)
...
const deleteSupply = async (supply) => {
  ...
  try {
    await apiClient.delete(`${SUPPLIES_PATH}/${supply.id}`)
