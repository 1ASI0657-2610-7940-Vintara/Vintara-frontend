...
const handleLogin = async () => {
  ...
  try {
    ...
    if (success) {
      // If remember me is set, we could store the email in localStorage for convenience (optional enhancement)
      if (rememberMe.value) {
      ...
      errorMsg.value = 'Credenciales incorrectas. Verifique su correo y contraseña.'
    }
  } catch (error) {
...
const handleGoogleSSO = () => {
  ...
  isLoading.value = true
  setTimeout(async () => {
    // Authenticate with default admin credentials via store simulation
    ...
    const success = await authStore.login({ email: adminEmail, password: adminPassword })
    if (success) {
      router.push('/dashboard')
    } else {
      errorMsg.value = 'Error al iniciar sesión con Google SSO.'
      isLoading.value = false
    }
  }, 1000)
}
